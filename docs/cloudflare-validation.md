...
Status: local verification, successful GitHub Actions deploy and DNS cutover, deployed `workers.dev` preview, independent canonical-domain smoke, and one owned-mailbox confirmation/notification attempt on 2026-09-25. **The notification reached `sent` in D1 after one provider-binding attempt; Outlook inbox delivery, arbitrary-recipient entitlement, and worldwide DNS propagation are not established by these results.**
...
The new `atelier-worker-launch-2026-09-25` campaign contained editable HTML and `send:true`. CI accepted it; remote D1 first showed one queued recipient. At `sent_at=1790340059285` (2026-09-25 12:40:59.285 UTC), the campaign was `complete` and its sole delivery was `sent`, `attempts=1`, with no recorded error. This is evidence that the scheduled dispatch path ran and the Cloudflare Email Sending binding returned success for **this Cloudflare-verified mailbox**. It is **not** evidence that Outlook placed the message in the inbox: await explicit user confirmation of receipt and inspect actual HTML/text plus one-click headers. Because this mailbox was Cloudflare-verified, it also does **not** establish Workers Paid entitlement or delivery to arbitrary subscriber addresses. No unsubscribe POST was exercised.
...
- One owned-mailbox subscription, confirmation, and one scheduled send are now observed. Unsubscribe POST, Outlook inbox placement, message HTML/text and one-click headers, Cloudflare Email Sending paid entitlement and arbitrary-recipient eligibility still require evidence. A `sent` D1 row records the Worker's successful provider-binding call, not inbox delivery. Canonical-domain smoke does not prove globally completed DNS propagation.

Verdict: **CI deployment, DNS proxy cutover, local compatibility, preview/canonical reading and search routes, owned-mailbox consent, and one provider-accepted scheduled update verified. Outlook inbox delivery, arbitrary-recipient entitlement, unsubscribe handling in production, and global DNS propagation remain unverified by this record**.
