...
Status: design implemented and remotely deployed on 2026-09-26. [First Action 36177202232](https://github.com/kleedaisuki/kleedaisuki.github.io/actions/runs/36177202232) applied additive D1 migration 0002 before deploying its Worker, then failed at draft dispatch. [Successful follow-up Action 36178715055](https://github.com/kleedaisuki/kleedaisuki.github.io/actions/runs/36178715055) reported `No migrations to apply!`, deployed the final Worker, and left the new-format notification **`send:false` with zero deliveries**. **Received-client rendering of the new templates has not been tested.** This note preserves the rationale and implementation contract for subscription-confirmation/update/action-page consistency. Product language and colors are defined in [atelier-email-design.md](atelier-email-design.md); client-compatibility research is in [email-client-compatibility.md](email-client-compatibility.md). The change does not alter Cloudflare's bounce `Return-Path`, consent state, or already-sent mail.

### Implemented source snapshot

[`worker/src/mail.rs`](../worker/src/mail.rs) now renders a shared confirmation/update mail shell, with a substantive localized confirmation text part and a fixed update footer in HTML and text. [`worker/src/domain.rs`](../worker/src/domain.rs) and the protected Worker API accept explicit `atelier-fragment-v1` input while omitted/`document` retains legacy full-document behavior. [`worker/migrations/0002_campaign_text.sql`](../worker/migrations/0002_campaign_text.sql) adds nullable `campaigns.text`; Rust stores complete HTML/text snapshots before queueing and uses the old text fallback for `NULL` legacy rows. [`scripts/dispatch-notifications.mjs`](../scripts/dispatch-notifications.mjs) handles optional `format`/`text_file` and selects referencing manifests on text-only edits. [`notifications/atelier-update.json`](../notifications/atelier-update.json) is a new-format **`send:false` draft** with issue-specific HTML fragment and authored text. Remote D1 now holds it as `draft` with HTML length 3473 and text length 689, both containing the unsubscribe placeholder and the 420-configuration summary; deliveries remain zero. The historical launch campaign remains `complete` with one delivery. This verifies persisted no-send snapshot behavior, **not** that the new template has been sent or rendered correctly by real mail clients.
...
Action 36177202232 applied additive D1 migration 0002 at 19:06:37 UTC **before** deploying its Worker; its later dispatch failed. The follow-up Action 36178715055 had no migration left to apply before deploying Worker version `3b80ccce-0724-4730-bff8-83ab7f0e12a7`. Retain migration-before-Worker ordering for later releases. Cloudflare documents that failed migrations roll back as units, but this does not substitute for a backup and local rehearsal ([D1 migrations](https://developers.cloudflare.com/d1/reference/migrations/), [Wrangler commands](https://developers.cloudflare.com/d1/wrangler-commands/)). If this Worker fails, roll back to the prior Worker: extra nullable `text` is harmless to prior queries. Do not remove the column or reinterpret queued snapshots during rollback. The release left historical `send:true` campaigns unchanged and only updated a `send:false` draft; review and queue a genuinely new issue under a fresh ID after recipient-client QA. Mail-client support is fragmented: Gmail supports much CSS but may ignore unsupported properties, so essential layout and colors stay inline and readable without enhancements ([Gmail CSS support](https://developers.google.com/workspace/gmail/design/css)).
