---
title: "Vibe Coding: Forging a Standard-Library-Flavored Stack in 40 Minutes"
description: "GPT-5 shows a genuine command of interfaces, type constraints, and allocator semantics."
date: "2025-08-25"
locale: "en"
...
Where, precisely, does GPT-5's engineering competence now stand?

Rather than staging the question rhetorically, here is the artifact produced after a 40-minute collaboration with it.

The code should, in principle, be released under GPLv3.0. Since I cannot conveniently create a GitHub repository and provide a copy at the moment, the license text is available here: <https://www.gnu.org/licenses/>

The prompts used during the exchange are as follows:

```prompt
Let us implement a Stack class targeting C++23.
...
```prompt
The current implementation is a solid basis; let us iterate on it:
...
```prompt
Let us narrow the scope for a moment and fix bugs:
[compiler log ...]
...
```prompt
Keeping the design requirements unchanged (that is, preserving the existing semantic constraints), continue fixing the bugs:
[compiler log ...]
```

```prompt
The current result is already rather substantial.
...
```prompt
The test code still contains a problem. Please fix the bug:
[compiler log ...]
```
